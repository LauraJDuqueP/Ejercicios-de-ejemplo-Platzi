pesos = input ("¿Cúantos pesos colombianos tienes?: ")
pesos = float(pesos)
valor_dolar = 3875 
dolares = (pesos/valor_dolar) 
dolares = round(dolares, 2)
dolares = str(dolares)
valor_libras = 5221 
libra_esterlina = (pesos/valor_libras)
libra_esterlina = round(libra_esterlina, 2)
libra_esterlina = str(libra_esterlina)

print("Tienes  $" + dolares +   "  dolares"  )
print ("Tienes $" + libra_esterlina +   "  libras esterlinas")

print ("esto es para el repositorio cambio 1")
print ("este es para el segundo ejemplo del repositorio")
print ("este es para el 3 ejemplo en el repo donde se guardan los cambios que se hicieron en github 2")
print ("segunda linea para el ejemplo del pull 1")
print ("Esto es para hacer el ejemplo donde se creo una rama y la estoy trabajando en ejederama")